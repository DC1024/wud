      ...
      <v-card-text class="pa-4">
        ...
        >
          <div class="text-center">
            <div class="text-caption text-grey mb-1">{{ $t('containers.currentVersion') }}</div>
          ...
          <div class="text-center">
            <div class="text-caption text-grey mb-1">{{ $t('containers.availableVersion') }}</div>
        ...
        >
          <div class="d-flex align-center">
            ...
            <div>
              <div class="text-caption text-grey">{{ $t('containers.targetTrigger') }}</div>
        ...
        <v-select
          v-else
          :label="$t('containers.triggerToExecute')"
...
export default defineComponent({
  ...
  computed: {
    containerDisplayName(): string {
      return this.container?.displayName || this.container?.name || this.$t("containers.unknownContainer");
    },
    containerImageName(): string {
      return this.container?.image?.name || this.container?.image?.path || "";
    },
    currentVersion(): string {
      if (!this.container) return this.$t("containers.unknownVersion");
      ...
      return this.$t("containers.unknownVersion");
    },
    displayNewVersion(): string {
      ...
      if (!this.container) return this.$t("containers.unknownVersion");
      let version = this.$t("containers.unknownVersion");
      if (
  ...
  methods: {
    ...
    async confirmUpdate() {
      ...
      try {
        ...
        bus?.emit(
          "notify",
          this.$t("containers.updateOk", { name: this.containerDisplayName }),
        );
        ...
        bus?.emit(
          "notify",
          this.$t("containers.updateError", { msg: errorMessage }),
          "error",
        );
      } finally {
